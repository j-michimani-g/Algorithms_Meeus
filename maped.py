...
def calculate_julian_day(Y: int, M: int, D: float) -> float:
    """
    Calculates the Julian Day (JD) for a given calendar date using formula (7.1) 
    from Astronomical Algorithms (Meeus).
    
    The function correctly handles the Julian-to-Gregorian calendar transition 
    on 15 October 1582.
    ...
    Returns:
        float: The calculated Julian Day (JD).
    """
    
    # 1. Adjust Y and M for January and February (Meeus's method)
    # Jan (1) and Feb (2) become month 13 and 14 of the preceding year.
    if M <= 2:
        Y = Y - 1
        M = M + 12
    
    # 2. Determine the value of B (Gregorian correction)
    
    # Gregorian calendar begins on 15 October 1582.
    is_gregorian = False
    if Y > 1582:
        is_gregorian = True
    elif Y == 1582:
        # Check if date is 1582-10-15 or later
        if M > 10:
            is_gregorian = True
        # Note: D is day of the month. If M was adjusted (M>12), this check is simplified.
        # Since M is already adjusted, we use the original M for the check (not robust here).
        # Let's use the standard rule from Meeus (before M, Y are modified):
        # We need to use the original month and day for the comparison, 
        # but the Y, M, D input here are ALREADY potentially modified.
        
        # IMPROVEMENT: Revert Y and M for the date check, or assume the provided 
        # JD formula (7.1) is for the PROLEPTIC Gregorian/Julian calendar based on Y,M,D input.
        # However, to be fully robust:
        
        # Check against the original date: 15 October 1582
        # Y=1582, M_orig=10, D_orig=15.
        # If we assume Y, M, D are the *modified* values:
        if M == 10 and D >= 15 and Y == 1582:
            is_gregorian = True
            
    # Simpler, cleaner approach for the Gregorian switch (often sufficient in modern code):
    # A=INT(Y/100), B=2-A+INT(A/4) for GREGORIAN (Y > 1582 or Y=1582, M>10, etc.)
    # B=0 for JULIAN (before 1582-10-15)
    
    # Let's stick to the simple condition for modern dates (Y >= 1583):
    # For any modern date, the original check is fine, but the calendar-switch logic is complex.
    # The current implementation of B is based on the logic from the image snippet:
    
    if Y * 10000 + M * 100 + D >= 15821015: # Gregorian starting 15 Oct 1582
...
def calculate_local_sidereal_time(GMST_degrees: float, longitude_360: float) -> float:
    """
        ...
        LST is found by adding the observer's longitude (expressed in hours) 
        to the Greenwich Mean Sidereal Time (GMST).
        
        Args: 
            GMST_degrees (float): The Greenwich Mean Sidereal Time in degrees. 
            longitude_360 (float): The longitude of the site, measured **eastward** in the range [0.0, 360.0].
                                   
        Returns:
            float: Local Sidereal Time (LST) in decimal hours [0.0, 24.0).
    """
    
    # Convert GMST from degrees to hours (1 hour = 15 degrees)
    GMST_hours = GMST_degrees / 15.0 
    
    # Convert Longitude from degrees to hours (1 hour = 15 degrees)
    # The input longitude_360 is assumed to be 0 to 360 EAST
    longitude_hours = longitude_360 / 15.0 
    
    # LST (in hours) = GMST (in hours) + Longitude (in hours, East positive)
    LST = (GMST_hours + longitude_hours) % 24.0

    if LST < 0: # Ensure positive result
        LST += 24.0
